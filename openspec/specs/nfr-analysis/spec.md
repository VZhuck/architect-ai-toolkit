...
- **WHEN** the caller invokes a command without `--nfr-path`, `.env` has no `NFR_PATH` value, and `sad/` contains markdown documents
...
- **WHEN** `/aait:nfr-detect` is invoked with no `path` argument
...
`/aait:nfr-detect` and `/aait:nfr-refine` SHALL write only the draft and trace files. `/aait:nfr-publish` SHALL render, run the validator, and promote to `nfrPath` only when validation passes. The skill SHALL NOT require commands to be run in order; a premature publish SHALL fail validation rather than write an incomplete document.

#### Scenario: Detect run alone

- **WHEN** `/aait:nfr-detect` completes
- **THEN** the draft and trace files exist and the target document is unchanged

#### Scenario: Publish before refining

- **WHEN** `/aait:nfr-publish` runs against a draft with unmeasured requirements and unset priorities
- **THEN** validation fails, the failures are reported, and the target document is unchanged

#### Scenario: Validation passes

- **WHEN** the rendered draft passes validation
- **THEN** the skill shows the diff against the current target document and promotes the draft to `nfrPath`

### Requirement: Provenance is recorded outside the deliverable

The skill SHALL record each requirement's verbatim source evidence and location, open questions, and dropped candidates in the trace file, and SHALL NOT place them in the target document. The pipeline SHALL remain operable if the trace file is absent.
...
- **WHEN** the trace file has been deleted and `/aait:nfr-publish` runs
