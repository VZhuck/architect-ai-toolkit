...
After all files are mined, the skill SHALL merge the per-file results in the main thread: assign final `NFR ID`s, merge duplicates of the same requirement across files into one entry that keeps every citation, and record each case where two sources give different targets for the same type and category as a row in `## Conflicts` with both values and sources. It SHALL NOT silently choose between them. Both conflicting entries SHALL become `TO REVIEW`, and the skill SHALL add one open question per conflict that references both `NFR ID`s; resolving the conflict closes that question. Only the main thread SHALL write the run's registry and state files.
...
- **AND** both entries are `TO REVIEW`, and one open question references both of their `NFR ID`s
...
- Tabs SHALL come in this order: each open question (except a question that only mirrors a conflict, which the conflict's tab answers), each conflict, `Priority` (only if the entry has none), and `Decision`. The submit step comes last.
...
- conflict resolved by picking a value: the winning entry keeps its value, and the losing entry moves to `## Dropped` with `Dropped by` = `USER` and a "superseded" reason
- conflict resolved by keeping both: both entries stay, and the resolution is recorded in their `Comments`
- a resolved conflict leaves `## Conflicts`, and its open question closes with the resolution
