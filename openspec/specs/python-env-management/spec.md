# python-env-management Specification

## Purpose

Manage Python dependencies and script execution for this repository via `uv`, so any Python script can be run with zero manual setup steps, superseding the prior `requirements.txt`-based workflow.
...
- **THEN** it declares at least `pypandoc>=1.11` and `langchain-text-splitters>=0.3.0`, matching `requirements.txt`'s prior contents, plus any new dependency the `archy-word-to-md` skill requires
...
- **WHEN** a user with `uv` installed but no existing `.venv` runs `uv run python skills/archy-word-to-md/scripts/docx_to_md.py --source <docx>`
