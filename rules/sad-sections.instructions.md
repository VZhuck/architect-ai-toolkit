...
paths: 
  - "/sad/**/*.md"
  - "/architecture/**/*.md
description: "style guide for markdown files in SAD (Solution Architecture Document) folders."
---
...
Files must use title-case-hyphenated titles with a section number prefix for reading order.
...
Naming rules:
- Use hyphens to separate words, preserving the source heading's word casing (Title-Case-Hyphenated, e.g. `Executive-Summary`) — do NOT lowercase the title. This is not kebab-case in the traditional (all-lowercase) sense.
- Strip any leading section numbering from the source heading text (e.g. a heading "1. Executive Summary" becomes the title `Executive-Summary`, not `1-Executive-Summary`) — the file's own `<n>.` prefix already encodes the order.
...
## Quality Checks (Before finalizing files)
