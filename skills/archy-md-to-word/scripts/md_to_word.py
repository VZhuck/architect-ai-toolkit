...
files from a source folder (as archy-word-to-md produces them), splits 00.Index.md
...
Usage:
  uv run python skills/archy-md-to-word/scripts/md_to_word.py --source sad
  uv run python skills/archy-md-to-word/scripts/md_to_word.py --source sad --template path/to/template.docx
  uv run python skills/archy-md-to-word/scripts/md_to_word.py --source sad --output dist/sad.docx
"""
...
def _splice_html_tables(
    ...
    as HTML by archy-word-to-md for colspan/rowspan or nested tables) are otherwise
