...
```bash
uv run python skills/archy-word-to-md/scripts/docx_to_md.py --source {sourceDoc} --target-folder {targetFolder}
```
...
- `skills/archy-word-to-md/scripts/table_normalizer.py` exposes `normalize_table_html(table_html: str) -> str` as a standalone, unit-testable function — feed it any HTML table string to see the conversion decision independent of a real docx.
- `skills/archy-word-to-md/scripts/sad_structure.py` exposes the cover/TOC split, H1 splitting, filename sanitizer, and anchor generator as standalone functions.
- `skills/archy-word-to-md/scripts/docx_to_md.py` wires both stages together and is the CLI entry point; `convert(source, target_folder, workdir=None)` is also importable directly.
- Tests: `uv run pytest skills/archy-word-to-md/tests/`.
